...
# Input Image Layer
def to_Input(name, file, to='(-3,0,0)', size=[8, 8]):
    return r'''
...
# image
def to_Image(name, file, to, size=[8, 8]):
    return r'''
\node[canvas is zy plane at x=0] ('''+ name +''') at (''' + to + ''') {\includegraphics[width='''+ str(size[0])+'cm'+''',height='''+ str(size[1])+'cm'+''']{'''+ file +'''}};
'''

def to_Grid(name, to, size=[4, 4], steps=1):
    return r'''
\node[canvas is zy plane at x=0] ('''+ name +''') at ('''+ to +''') {\drawgrid{'''+ str(size[0]) +'''}{'''+ str(size[1]) +'''}{'''+ str(size[1]/steps) +'''}};
'''
...
# Conv,relu
def to_Upsample(name, s_filter='', n_filter=(), offset='(0,0,0)', to='0,0,0', width=(2), size_1=(40,40), size_2=(80,80), caption=' '):
    if isinstance(width, list):
        if(len(n_filter) != len(width)):
            warnings.warn('Size of n_filters does not match size of width');
        xlabel_string = listToString(n_filter)
...
# Conv
def to_Shortcut(name, s_filter='', n_filter=(64, 64), offset='(0,0,0)', to='0,0,0', width=(1), size=[40, 40], caption=' '):
    if isinstance(width, list):
        if(len(n_filter) != len(width)):
            warnings.warn('Size of n_filters does not match size of width');
        xlabel_string = listToString(n_filter)
...
def to_LongConnection(of, to, pos=1.25, anchor_of_1='-south', anchor_of_2='-north', anchor_to='-north'):
    return r'''
\path ('''+ of + anchor_of_1+''') -- ('''+ of + anchor_of_2 +''') coordinate[pos='''+ str(pos) +'''] ('''+ of +'''-dummy) ;
\path ('''+ of +'''-dummy |- '''+ to + anchor_to +''') coordinate ('''+ to +'''-dummy)  ;
...
def to_Ellipsis(of, to, offset='(0.5,0,0)'):
    return r'''
\draw [connection] ('''+ of +'''-east) -- node [fill=white,inner sep=1pt, opacity=1] {\ldots} ('''+ to +'''-west);
'''
