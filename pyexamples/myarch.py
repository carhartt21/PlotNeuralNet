...
def creatArchitecture(arch):
    ...
    # arch += conv_relu(name='conv_0', s_filter='I', size=(input, input), caption='1', n_filter=32)
    arch += conv_relu(name='conv_0', s_filter='I', size=(input, input), caption='0', n_filter=32)
    arch += [*multi_conv_relu(num=3, name='conv', prev='conv_0', layer_num=1, name_start=1, n_filter=[64, 32, 64], s_filter='I/2', offset = '(3,0,0)', scale=16, size=(input/2, input/2), conn=True)]
    ...
    arch += [*shortcut(name='short_8', prev='conv_7', size=(input/4, input/4), caption='8', s_filter='I/4'), to_LongConnection('conv_5', 'short_8', pos=1.5), to_ShortConnection(of='conv_7', to='short_8')]
    # check in cfg
    arch += [*shortcut(name='short_11', prev='short_8', size=(input/8, input/8), caption='11', s_filter='I/8')]
    # arch += [*conv_relu(name='conv_12', prev='short_8', s_filter='I/8', n_filter=(256), size=(input/8, input/8), caption='12')]
    arch += ellipsis(of='short_8', to='short_11')
    # arch += [*conv_relu(name='conv_37', prev='conv_19', s_filter='I/16', n_filter=(512), size=(input/16, input/16), caption='37')]
    ...
    # arch += [*conv_relu(name='short_61', prev='conv_37', s_filter='I/32', n_filter=(1024), size=(input/32, input/32))]
    ...
    arch += [*upsample(name='conv_86', prev='conv_85', s_filter='I/16', n_filter=(256), offset='(1,0,0)', size=(input/32, input/32), caption='86', conn=True)]
    ...
    arch += [*upsample(name='conv_97', prev='conv_96', s_filter='I/8', n_filter=(128), offset='(1,0,0)', size=(input/16, input/16), caption='97', conn=True)]
    ...
    arch += [*upsample(name='conv_109', prev='conv_108', s_filter='I/4', n_filter=(128), offset='(1,0,0)', size=(input/8, input/8), caption='109', conn=True)]
    ...
    arch += [*upsample(name='conv_121', prev='conv_120', s_filter='I/2', n_filter=(128), offset='(1,0,0)', size=(input/4, input/4), caption='121', conn=True)]
    ...
    # arch += shortcut
    arch += legend()
    # arch += [*conv(name='conv_80', prev='conv_79', s_filter='I/32', n_filter=(512), offset='(1,0,1)', size=(1, 1), caption='80')]
    # arch += [*conv(name='conv_81', prev='conv_80', s_filter='I/32', n_filter=(512), offset='(1,0,1)', size=(1, 1), caption='81')]
    arch += env_end()
    return arch
